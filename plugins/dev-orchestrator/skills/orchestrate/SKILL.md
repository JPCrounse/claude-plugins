---
name: orchestrate
description: Manage a large, multi-step development effort as a structured 5-phase workflow — goal definition, context collection, roadmap generation, phased implementation, and final review — with all progress persisted to `.dev-orchestrator/` files for cross-session resumption and a choice of supervised or one-shot autonomy. Use this skill when the user wants to "start a development workflow", "break down a large story", "plan and implement a feature", "orchestrate development phases", "create a development roadmap", "implement in phases", "implement this epic", or "plan this sprint work". Also use it to resume an existing dev-orchestrator workflow — "continue where I left off", "pick up where we stopped", "where are we on the implementation", "what's the status of my workflow", "resume the implementation" (these resume triggers assume a `.dev-orchestrator/` workflow already exists). Apply proactively only for genuinely large efforts — work spanning multiple subsystems, likely to exceed ~10 implementation steps, or expected to run across multiple sessions — even if the user doesn't explicitly ask for "orchestration". Do NOT use for simple single-file fixes, small tasks that fit comfortably in one session, code reviews, or code explanation requests.
---
...
4. Write `manifest.json` with topics, timestamps, `currentPhase: "context-collection"`, and a zero-initialized `metrics` object (`agentInvocations`, `subAgentSpawns`, `phasesImplemented`, `clustersProcessed` — all `0`). Schema: `references/state-file-formats.md`. Do not yet set `executionMode`, `acceptanceMode`, or `guidanceCollectionMode` — those are set in Phase 1.5 and Phase 2.
...
Surface the cluster numbers from the roadmap-generator's summary: total phases, total clusters, the multi-phase vs. singleton split, and the **shared-context reloads avoided** estimate (Σ over multi-phase clusters of phases−1). Frame the choice with that number — e.g. *"efficiency mode avoids ~N shared-context reloads on this roadmap; if N is 0, speed mode costs no more and is strictly faster."* Ask the user to choose:
...
- **One-shot mode** — Same cluster-based dispatch as efficiency, but pass a one-shot directive to the inner agent (logs to `one-shot-log.md`, parallelizes `[concurrent]` groups). Emit a one-line status message at each phase/cluster start and end — the only user-visible signal mid-Phase-4. **Agent-spawn ceiling:** before each delegation, check total agents spawned so far (`metrics.agentInvocations + metrics.subAgentSpawns`, the latter accumulated from returned handoffs) against a soft ceiling of `max(20, 3 × total roadmap phases)`; if it is exceeded before all phases complete, append `[BUDGET CEILING]` to `one-shot-log.md`, set `currentPhase: "final-review"`, and stop — the runaway-cost backstop for unattended runs (one-shot cannot pause for input, so it aborts and surfaces the ceiling at Phase 5). For the full per-aspect one-shot delta, see the One-Shot Mode Cheat Sheet in `references/workflow-phases.md`.

**Metrics (all modes).** On every delegation, increment `metrics.agentInvocations` in `manifest.json`. When a handoff reports sub-agents spawned, add them to `metrics.subAgentSpawns`; bump `metrics.phasesImplemented` and `metrics.clustersProcessed` as those units complete. These counts are the workflow's cost proxy (the skill has no token meter) and are surfaced by `status-reviewer` and at Phase 5.
...
1. **Subagent delegation (primary).** Heavy work (file reads, edits, sub-sub-agents) runs inside `guidance-collector`, `roadmap-generator`, `phase-implementer`, `cluster-implementer`, `status-reviewer`, and `final-reviewer` — each in its own context window. Only structured handoff summaries (target ~1–2K tokens) return to the orchestrator thread.
2. **Two-layer isolation in efficiency and one-shot multi-phase clusters.** `cluster-implementer` (outer) reads shared context once, then delegates each phase to a nested `phase-implementer` (inner). Per-phase implementation residue never accumulates in the outer cluster context, and cluster handoffs digest settled phases rather than carrying every phase verbatim. Speed mode and singleton clusters skip the wrapper.
3. **File-based state.** All progress is persisted to `.dev-orchestrator/`. The skill never depends on conversation history. After auto-compaction in supervised modes, re-read state files and continue. In one-shot mode, the running agent re-reads `one-shot-log.md` to identify where it was. Resuming agents read only the **tail** of any session log (recent entries + unresolved blocking entries), never the whole append-only log — see Bounded reads in `references/state-file-formats.md`.
4. **Prompt-cache preservation (maintainer guard).** Agent system prompts and the orchestrator's task-brief templates must stay **static** — no timestamps, run/session IDs, or dynamically-assembled tool/context lists in the prefix. Per-invocation variables (topic, phase, paths, directives) go in the task-brief suffix, after the cacheable prefix. This is what lets a repeatedly-invoked agent like `phase-implementer` reuse its cached prompt across calls (a cache read costs ≈10% of fresh input). One stray timestamp in a prompt prefix silently forfeits that discount on every call — do not introduce one.
5. **Cost visibility.** The skill maintains `metrics` counters in `manifest.json` (`agentInvocations`, `subAgentSpawns`, `phasesImplemented`, `clustersProcessed`) as a consumption proxy, since the harness exposes no token meter. `status-reviewer` reports them on request and `final-reviewer` reports them at completion; one-shot mode enforces a soft agent-spawn ceiling (Phase 4) as a runaway backstop. Measurement is deliberately paired with the speed-vs-efficiency choice at Phase 3.5, framed with the roadmap-generator's concrete reload-savings estimate.
